...
check(cmd(mtn3("setup", "--branch=testbranch", ".")), 0, false, false)
writefile("otherfile", "baz quux")
check(cmd(mtn3("add", "otherfile")), 0, false, false)
...
function chksy(n, co_mtn)
  check_same_stdout(cmd(mtn2("automate", "get_revision", revs[n])),
                    cmd(mtn3("automate", "get_revision", revs[n])))
  check_same_stdout(cmd(mtn2("ls", "certs", revs[n])),
                    cmd(mtn3("ls", "certs", revs[n])))
  remove_recursive("somedir")
  check(cmd(co_mtn("checkout", "--revision", revs[n], "somedir")), 0, false, false)
end
...
remove_recursive("_MTN")
check(cmd(mtn2("checkout", "--revision", revs[1], ".")), 0, false, false)
