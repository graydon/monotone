	     ...
	     string fragment)
{
  ...
  uri u;
  BOOST_CHECK_NOT_THROW(parse_uri(built, u), informative_failure);
  BOOST_CHECK(u.scheme == scheme);
  BOOST_CHECK(u.user == user);
  if (!normal_host.empty())
    BOOST_CHECK(u.host == normal_host);
  else
    BOOST_CHECK(u.host == ipv6_host);
...
UNIT_TEST(uri, invalid)
{
  ...
  BOOST_CHECK_THROW(parse_uri("http://[f3:03:21/foo/bar", u), informative_failure);
  BOOST_CHECK_THROW(parse_uri("http://example.com:/foo/bar", u), informative_failure);
  BOOST_CHECK_THROW(parse_uri("http://example.com:1a4/foo/bar", u), informative_failure);
}
