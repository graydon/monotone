/* Formerly libpcre config.h.in.  We do not need to worry about most of the
   portability issues that upstream libpcre's build system concerns itself
   with, so we make no attempt to wire this up to monotone's configure.  */

/* Make sure these options are disabled, as we did not import all of their
   support code.  */
#undef SUPPORT_UTF8
#undef SUPPORT_UCP
#undef DEBUG
